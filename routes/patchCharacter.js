...
router.patch("/characters/:id", async (req, res) => {
  ...
  const patchingObject = await charactersRM.findOneAndUpdate(
    patchQuery,
    patchObject,
    patchOptions
  );
  ...
  patchingObject.lastErrorObject.updatedExisting
    ? res.json(patchedObject)
    : res.status(500).json({ error: "failed to patch. try again later." });
});
