...
const { checkEmptyInput, sanitizeObjectInput, checkId, doesItExist} = require('../functionsLibrary');
...
router.put('/characters/:id', async (req, res) => {
    ...
    const updatingObject = await charactersRM.findOneAndUpdate(updateQuery, updateObject, updateOptions);
    const updatedObject = await charactersRM.findOne(updateQuery);

    await dbclose();

    updatingObject.lastErrorObject.updatedExisting ? res.json(updatedObject) : res.status(500).json({"error": "failed to update. try again later."});
})
